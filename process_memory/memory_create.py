...
def _persist_event(db, event, header):
    ...
    new_event['referenceDate'] = util.get_datetime_from(event.get('referenceDate', None))
...
def _create_header_object(json_data, app_name):
    ...
    new_header['referenceDate'] = util.get_datetime_from(event.get('referenceDate', None))
